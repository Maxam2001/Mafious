...
//item definitions (ไม่ใช้ รอเอาออก)
var items = {
	gun: {
		name: 'Handgun',
...
var playerRoles = [];	//สร้าง array สำหรับ role สำหรับ custom ?
...
// custom settings (น่าจะไม่ใช้ ถ้าไม่ใช้ก็เอาออก)
if (argv.custom) {
	...
	if (state !== 3) {	//ถ้ายังไม่จบเกม
		if (ticksLeft && !endDay) {	//ยังเหลือเวลา และยังไม่จบกลางวัน
			...
			if (state !== 3) {	//และถ้าเกมยังไม่จบ
				...
				updateHeader('Night ' + nightCount);	//ข้อความด้านบนก็เปลี่ยนเป็นกลางคืน
				updateAnnouncement('It is now nighttime');	//แจ้งทุกคนว่ากลางคืนแล้ว

				io.sockets.emit('clearTargets');	// ?
				io.sockets.emit('displayInventory', false);		//inventory ไม่ได้ใช้ รอเอาออก
	...
	if (state !== 3) {	//ถ้าเกมยังไม่จบ
		if (ticksLeft && !endDay) {	//ถ้าเวลาเหลือ หรือกลางคืนยังไม่จบ
			...
			if (state !== 3) { //surely there's a cleaner way to do this
				...
				updateHeader('Day ' + dayCount);	//ประกาศกลางวันที่
